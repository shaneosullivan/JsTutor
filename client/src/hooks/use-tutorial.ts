...
export function useTutorial() {
  ...
  useEffect(() => {
    ...
    } else if (progress && typeof progress === 'object' && progress !== null && 'completedTutorials' in progress) {
  ...
  const selectTutorial = (tutorial: Tutorial) => {
    ...
    const isUnlocked = tutorial.order === 1 || !previousTutorial || completedTutorials.includes(previousTutorial.id);
