...
const CodeEditor = forwardRef<CodeEditorRef, CodeEditorProps>(
  function CodeEditor(
    ...
    const [hasInitiallyFormatted, setHasInitiallyFormatted] = useState(false);
    ...
    const calculateEditorCharacterWidth = (): number => {
      ...
      measureElement.style.font =
        "14px 'Fira Code', 'Monaco', 'Menlo', monospace";
    ...
    useEffect(() => {
      if (viewRef.current && value !== viewRef.current.state.doc.toString()) {
        // Check if this is a significant change (like switching tutorials)
        // Reset formatting flag if the new value is very different from previous
        if (
          previousValueRef.current &&
          value.length > 0 &&
          previousValueRef.current.length > 0 &&
          !value.includes(
            previousValueRef.current.substring(
              0,
              Math.min(50, previousValueRef.current.length)
            )
          )
        ) {
          setHasInitiallyFormatted(false);
        }
        ...
        // Format code on initial load if formatting is enabled and we haven't formatted yet
        if (shouldFormat && !hasInitiallyFormatted && value.trim()) {
          setHasInitiallyFormatted(true);
          // Use a small delay to allow the editor to fully initialize
          setTimeout(async () => {
            await performFormat();
          }, 100);
        }
      }
    }, [value, shouldFormat, hasInitiallyFormatted]);
